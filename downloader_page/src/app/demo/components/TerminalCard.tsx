    ...
    <div className="group relative flex h-full min-h-[420px] flex-col overflow-hidden rounded-xl border border-zinc-200/70 bg-white/80 transition hover:border-zinc-300 dark:border-white/[0.07] dark:bg-zinc-950/70 dark:hover:border-white/[0.14]">
      <div className="flex items-center justify-between gap-2 border-b border-zinc-200/70 px-4 py-2.5 dark:border-white/[0.05]">
        <div className="flex min-w-0 items-center gap-2">
          <span className={`${cli.color} shrink-0 font-mono text-[21px] leading-none`}>
            {cli.glyph}
          </span>
          <div className="min-w-0 leading-tight">
            <div className="truncate text-[15.5px] text-zinc-900 dark:text-white">{cli.name}</div>
            <div className="truncate font-mono text-[12.5px] text-zinc-500">{model}</div>
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          ...
          <span
            className={`inline-flex items-center gap-1.5 rounded-md border px-1.5 py-0.5 text-[12px] uppercase tracking-wider ${s.cls}`}
          >
            {cli.state === "executing" && (
          ...
          <div className="relative">
            ...
            {menuOpen && (
              ...
              >
                <button className="flex w-full items-center gap-2 px-3 py-2 text-left text-[14.5px] text-zinc-700 hover:bg-zinc-50 dark:text-zinc-200 dark:hover:bg-white/5">
                  <Pause className="h-3 w-3 text-zinc-500" />
                  <span>Pause</span>
                  <span className="ml-auto font-mono text-[12px] text-zinc-400">⌘P</span>
                </button>
                <button className="flex w-full items-center gap-2 border-t border-zinc-200/70 px-3 py-2 text-left text-[14.5px] text-zinc-700 hover:bg-zinc-50 dark:border-white/[0.06] dark:text-zinc-200 dark:hover:bg-white/5">
                  <RefreshCw className="h-3 w-3 text-zinc-500" />
                  <span>Reload</span>
                  <span className="ml-auto font-mono text-[12px] text-zinc-400">⌘R</span>
                </button>
                <button className="flex w-full items-start gap-2 border-t border-zinc-200/70 px-3 py-2 text-left text-[14.5px] text-zinc-700 hover:bg-zinc-50 dark:border-white/[0.06] dark:text-zinc-200 dark:hover:bg-white/5">
                  <Maximize2 className="mt-0.5 h-3 w-3 shrink-0 text-zinc-500" />
                  <div className="min-w-0">
                    <div>Full-screen</div>
                    <div className="mt-0.5 font-mono text-[12.5px] leading-snug text-zinc-500">
      ...
      <div className="space-y-1.5 px-4 pt-3">
        ...
        <div className="flex items-center justify-between font-mono text-[12px] text-zinc-500">
      ...
      <div className="m-3 mt-3 flex flex-1 flex-col overflow-hidden rounded-lg border border-zinc-200/70 bg-zinc-50/60 dark:border-white/[0.05] dark:bg-black/50">
        ...
        <div className="flex-1 space-y-0.5 overflow-y-auto px-3 py-2 font-mono text-[14px] leading-relaxed">
          {cli.log.map((l, i) => (
        ...
        <div className="flex items-center gap-1 border-t border-zinc-200/70 bg-white px-2 py-1 dark:border-white/[0.04] dark:bg-black/30">
          <span className="font-mono text-[14px] text-zinc-400">$</span>
          <input
      ...
      {cli.state === "permission" && cli.pendingCmd && (
        <div className="border-t border-amber-300/40 bg-amber-50/70 px-4 py-2.5 dark:border-amber-400/15 dark:bg-amber-400/[0.04]">
          <div className="flex items-center gap-1.5 text-[13px] text-amber-700 dark:text-amber-200">
            <ShieldAlert className="h-3 w-3" />
            Approval required for destructive command
          </div>
          <pre className="mt-1.5 rounded border border-amber-300/40 bg-white px-2 py-1 font-mono text-[13px] text-amber-800 dark:border-amber-400/15 dark:bg-black/40 dark:text-amber-100">
            {cli.pendingCmd}
          </pre>
          <div className="mt-2 flex items-center justify-end gap-1.5">
            <button className="rounded-md border border-zinc-200/70 bg-white px-2 py-1 text-[13px] text-zinc-700 hover:bg-zinc-50 dark:border-white/10 dark:bg-white/[0.04] dark:text-zinc-300 dark:hover:bg-white/[0.08]">
              Deny
            </button>
            <button className="rounded-md bg-emerald-500 px-2 py-1 text-[13px] text-white hover:bg-emerald-600">
              Approve once
            </button>
            <button className="rounded-md border border-emerald-400/40 bg-emerald-100 px-2 py-1 text-[13px] text-emerald-700 hover:bg-emerald-200 dark:border-emerald-400/30 dark:bg-emerald-400/10 dark:text-emerald-300 dark:hover:bg-emerald-400/20">
              Always allow
            </button>
          </div>
        </div>
